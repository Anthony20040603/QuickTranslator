...
界面采用轻量的 Windows 记事本式布局：系统原生标题栏下面是 Windows 原生的“模型、主题、设置”菜单栏，展开和横向切换行为与 Everything 等传统 Windows 软件一致。主体是不带卡片和边框的纯色文本编辑区，底部保留确认与复制按钮。程序图标用于 EXE、窗口标题栏和系统托盘。
